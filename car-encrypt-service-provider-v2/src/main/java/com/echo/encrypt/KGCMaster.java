...
public class KGCMaster {
    ...
    public void init() {
        ...
        if (keyPair.get(0) == null) {
            ...
            map.put("id", id);
            map.put("encMasterKeyPair", CodeUtil.encodeToString(encryptMasterKeyPair.toByteArray()));
            map.put("signMasterKeyPair", CodeUtil.encodeToString(signMasterKeyPair.toByteArray()));
            kgcMasterService.updateKeyPair(map);
        } else {
            byte[] encMasterKeyPairByte = CodeUtil.decodeStringToByte(keyPair.get(0).get("enc_master_key_pair"));
            byte[] signMasterKeyPairByte = CodeUtil.decodeStringToByte(keyPair.get(0).get("sign_master_key_pair"));
        ...
        Map<String, Object> map = new HashMap<>();
        map.put("publicEncMasterKey", CodeUtil.encodeToString(encryptMasterKeyPair.getPublicKey().toByteArray()));
        map.put("privateEncMasterKey", CodeUtil.encodeToString(encryptMasterKeyPair.getPrivateKey().toByteArray()));
        map.put("publicSignMasterKey", CodeUtil.encodeToString(signMasterKeyPair.getPublicKey().toByteArray()));
        map.put("privateSignMasterKey", CodeUtil.encodeToString(signMasterKeyPair.getPrivateKey().toByteArray()));
        map.put("id", id);
